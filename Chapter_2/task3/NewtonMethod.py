...
from Chapter_2.task3.AddictionFunctions import *
from Chapter_2.task3 import Swann, GoldenFusion
...
def main():
    ...
    if is_positive_definite(H):
        ...
        print(f"=> Определяем величину: d{k} = -1 * H(x{k}) * ▽f(x{k}) = {d} =>")
    ...
    while k < M and (norm(tuple(nx[i] - x[i] for i in range(2))) > eps2 or abs(fnx - fx) > eps2 or double_check < 1):
        ...
        if not (norm(tuple(nx[i] - x[i] for i in range(2))) > eps2 or abs(fnx - fx) > eps2):
            print(
                '------------------------------------------------------------------------------------------------------------------------')
            print(
                f'Выполняются условия: ||x{k + 1} - x{k}||={norm(tuple(nx[i] - x[i] for i in range(2)))} < eps2_{eps2} и |f(x{k + 1} - f(x{k})|={abs(func(nx) - func(x))} < eps2_{eps2} =>')
